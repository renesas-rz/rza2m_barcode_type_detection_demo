...
static uint8_t graphics3_buffer[GRAPHICS3_BUFFER_NUM][(GRAPHICS3_BUFFER_STRIDE * GRAPHICS3_BUFFER_HEIGHT)] __attribute__ ((section("Graphics_RAM")));
static volatile int32_t vsync_count = 0;

static volatile uint32_t Req_Layer1_Swap = 0;
static volatile uint32_t Req_Layer2_Swap = 0;
static volatile uint32_t Req_Layer3_Swap = 0;

static volatile uint32_t Layer1_buffer_id = 0;  //current display buffer id
static volatile uint32_t layer2_buffer_id = 0;
static volatile uint32_t layer3_buffer_id = 0;
...
uint8_t *R_BCD_LcdGetVramAddress(void)
{
    ...
    {
        paddr = &video_buffer[1-Layer1_buffer_id][0]; //Get non-display buffer ID
    }
#endif
    return paddr;
...
/*******************************************************************************
* Function Name: R_BCD_LcdGetOLVram2Address
* Description  : Acquire the address of the Overlap buffer which is not currently being displayed
* Arguments    : -
* Return Value : Overlap buffer address
*******************************************************************************/

uint8_t *R_BCD_LcdGetOLVram2Address(void)
{
    uint8_t *paddr;
	
#if GRAPHICS2_BUFFER_NUM == 1
    paddr = &graphics3_buffer[0][0];
#else
    if (Req_Layer3_Swap != 0)
    {
        paddr = &graphics3_buffer[layer3_buffer_id][0];
    }
    else
    {
        paddr =  &graphics3_buffer[1-layer3_buffer_id][0];
    }
#endif
    return paddr;
}
...
/*******************************************************************************
* End of function R_BCD_LcdGetOLVram2Address
*******************************************************************************/
...
int32_t R_BCD_LcdInit(void)
{
    ...
    if (error == VDC_OK)
    {
        ...
        line_num = LCD_CH0_DISP_VS + LCD_CH0_DISP_VW;//R_BCD_LCD_HEIGHT;
    ...
    /***************************************/
    /* Graphic Layer 3 VDC_GR_FORMAT_CLUT4 */
    /***************************************/
    ...
    if (error == VDC_OK)
    {
    	...
    	gr_disp_cnf.layer_id  = VDC_LAYER_ID_3_RD;              /* Layer ID                        */
    	gr_disp_cnf.fb_buff   = (void *)&graphics3_buffer[0];   /* Frame buffer address            */
    	gr_disp_cnf.fb_stride = GRAPHICS3_BUFFER_STRIDE;        /* Frame buffer stride             */

    	/* Display Area */
    	gr_disp_cnf.disp_area.hs_rel = LAYER3_DISP_AREA_HS;
    	gr_disp_cnf.disp_area.hw_rel = LAYER3_DISP_AREA_HW;
    	gr_disp_cnf.disp_area.vs_rel = LAYER3_DISP_AREA_VS;
    	gr_disp_cnf.disp_area.vw_rel = LAYER3_DISP_AREA_VW;
...

    {
        ...
        draw_buffer_info            info3;
        info3.buff_addr1       = graphics3_buffer[0];
        info3.buff_addr2       = (GRAPHICS3_BUFFER_NUM == 1) ? graphics3_buffer[0] : graphics3_buffer[1];
        info3.buff_stride      = GRAPHICS3_BUFFER_STRIDE;
        info3.window_width     = LAYER3_DISP_AREA_HW;
        info3.window_height    = LAYER3_DISP_AREA_VW;
        DrawInit(VDC_LAYER_ID_3_RD, &info3);

        /* buffer clear */
        DrawRectFill(VDC_LAYER_ID_2_RD, 0,  LAYER2_DISP_AREA_VS, R_BCD_LCD_WIDTH2, R_BCD_LCD_HEIGHT2, TRANSPARENT);
        DrawSwapBuffer(VDC_LAYER_ID_2_RD); //application will draw to back buffer at next time
        DrawRectFill(VDC_LAYER_ID_3_RD, 0,  0, R_BCD_LCD_WIDTH3, R_BCD_LCD_HEIGHT3, TRANSPARENT);
        DrawSwapBuffer(VDC_LAYER_ID_3_RD); //application will draw to back buffer at next time
    }
...
/*******************************************************************************
* Function Name: R_BCD_LcdSwapGraphicsBuffer2
* Description  : Record graphics buffer2 swap request
* Arguments    : -
* Return Value : -
*******************************************************************************/
void R_BCD_LcdSwapGraphicsBuffer2(void)
{
    Req_Layer3_Swap = 1;
...
/*******************************************************************************
* End of function R_BCD_LcdSwapGraphicsBuffer2
*******************************************************************************/

/*******************************************************************************
* Function Name: R_BCD_ChkLcdSwapVideoBufferReq
* Description  : Read video buffer swap request
* Arguments    : -
* Return Value : Information on switching request of layer 1 (video buffer)
*******************************************************************************/
uint32_t R_BCD_ChkLcdSwapVideoBufferReq(void)
{
    return Req_Layer1_Swap;
}
/*******************************************************************************
* End of function R_BCD_ChkLcdSwapVideoBufferReq
*******************************************************************************/

/*******************************************************************************
* Function Name: R_BCD_ChkLcdSwapGraphicsBufferReq
* Description  : Read graphics buffer swap request
* Arguments    : -
* Return Value : Information on switching request of layer 2 (graphic buffer)
*******************************************************************************/
uint32_t R_BCD_ChkLcdSwapGraphicsBufferReq(void)
{
    return Req_Layer2_Swap;
}
/*******************************************************************************
* End of function R_BCD_ChkLcdSwapGraphicsBufferReq
*******************************************************************************/

/*******************************************************************************
* Function Name: R_BCD_ChkLcdSwapGraphicsBuffer2Req
* Description  : Read graphics buffer swap request
* Arguments    : -
* Return Value : Information on switching request of layer 3 (graphic buffer)
*******************************************************************************/
uint32_t R_BCD_ChkLcdSwapGraphicsBuffer2Req(void)
{
    return Req_Layer3_Swap;
}
/*******************************************************************************
* End of function R_BCD_ChkLcdSwapGraphicsBuffer2Req
*******************************************************************************/
...
void R_BCD_LcdVSync(void) 
{
    ...
    if (Req_Layer1_Swap != 0  && VIDEO_BUFFER_NUM != 1)
    {
    ...
    /* Check whether the overlap buffer2 swap request is pending */
    if (Req_Layer3_Swap != 0  && GRAPHICS3_BUFFER_NUM != 1)
    {
        if (layer3_buffer_id == 0)
        {
            R_RVAPI_GraphChangeSurfaceVDC(VDC_CHANNEL_0, VDC_LAYER_ID_3_RD, graphics3_buffer[1]);
            layer3_buffer_id = 1;
        }
        else
        {
            R_RVAPI_GraphChangeSurfaceVDC(VDC_CHANNEL_0, VDC_LAYER_ID_3_RD, graphics3_buffer[0]);
            layer3_buffer_id = 0;
        }
        DrawSwapBuffer(VDC_LAYER_ID_3_RD);
    }
...
/*******************************************************************************
* Function Name: R_BCD_LcdWriteLine2
* Description  : Draw a line in the overlap buffer2
* Arguments    : x1
*              :   X coordinate of the start point of the line
*              : y1
*              :   Y coordinate of the start point of the line
...
* Return Value : -
*******************************************************************************/
void R_BCD_LcdWriteLine2(int32_t x1, int32_t y1, int32_t x2, int32_t y2, uint8_t color)
{
    DrawLine(VDC_LAYER_ID_3_RD, x1, y1, x2, y2, color);

    return;
}
/*******************************************************************************
* End of function R_BCD_LcdWriteLine2
*******************************************************************************/
...
/*******************************************************************************
* End of function R_BCD_LcdWriteChar_Ch
*******************************************************************************/

/*******************************************************************************
* Function Name: R_BCD_LcdWriteChar2
* Description  : Display character in overlap buffer2
* Arguments    : code
*              :   Code to be displayed
*              : x
*              :   X coordinate to display character
...
* Return Value : -
*******************************************************************************/
void R_BCD_LcdWriteChar2(uint8_t code, uint32_t x, uint32_t y, uint8_t color)
{
    int32_t height;
    int32_t width;
    uint8_t font;
    ...
    for (height = 0; height < R_BCD_FONTDATA_HEIGHT; height++)
    {
        ...
        for (width = 0; width < R_BCD_FONTDATA_WIDTH; width++)
        {
            if ((font & (0x1 << (check_bit--))) != 0)
            {
                DrawPoint(VDC_LAYER_ID_3_RD, width + x, height + y , color);
            }
            if (check_bit < 0)
            {
                check_bit = 7;
...
/*******************************************************************************
* End of function R_BCD_LcdWriteChar2
*******************************************************************************/
...
/*******************************************************************************
* Function Name: R_BCD_LcdWriteString2
* Description  : Display character string in overlap buffer2
* Arguments    : pcode
*              :   A pointer to the character string to be displayed
*              : x
*              :   X coordinate to display character string
...
* Return Value : -
*******************************************************************************/
void R_BCD_LcdWriteString2(uint8_t *pcode, uint32_t x, uint32_t y, uint8_t color)
{
    int i;
    int len;
    ...
    for (i = 0; i < len; i++)
    {
        R_BCD_LcdWriteChar2(*pcode++, (x + (i * R_BCD_FONTDATA_WIDTH)), y, color);
    }

    return;
}
/*******************************************************************************
* End of function R_BCD_LcdWriteString2
*******************************************************************************/
...
/*******************************************************************************
* Function Name: R_BCD_LcdClearGraphicsBuffer2
* Description  : Clear overlap buffer
* Arguments    : -
* Return Value : -
*******************************************************************************/
void R_BCD_LcdClearGraphicsBuffer2(void)
{
    memset(R_BCD_LcdGetOLVram2Address(), (int)0xffffffff, (GRAPHICS3_BUFFER_STRIDE * LAYER3_DISP_AREA_VW));

    return;
}
/*******************************************************************************
* End of function R_BCD_LcdClearGraphicsBuffer2
*******************************************************************************/
